        ...
        <div className={props.className}>
            <div className={'flex h-full justify-between items-center text-sm ' + (fullAlbumMode? 'px-5':'px-3')}>
                ...
                <span className={(fullAlbumMode? 'space-x-5' : 'space-x-3') + ' flex-none flex items-center'}>
                    ...
                    <Menu className={'text-md ml-2'} options={['Copy Link', 'Download']} fullAlbumMode={fullAlbumMode} linkToCopy={`${baseURL}/cds/${props.albumId}?song=${song.id}`} songname={song.titleRom}/>
                </span>
            </div>
        </div>
    )
